...
impl MdbxProofsStorage {
    ...
    fn store_trie_updates_inner(
        ...
        proof_window_cursor
            .append(ProofWindowKey::LatestBlock, &BlockNumberHash::new(block_number, block.hash))?;
        Ok(())
    }
}
...
impl OpProofsStore for MdbxProofsStorage {
    ...
    async fn store_trie_updates(
        ...
        self.env.update(|tx| {
            self.store_trie_updates_inner(tx, block_ref.block, block_state_diff)?;
            Ok(())
        })?
    }
    ...
    async fn replace_updates(
        ...
        self.env.update(|tx| {
            ...
            for (block_with_parent, diff) in blocks_to_add {
                self.store_trie_updates_inner(tx, block_with_parent.block, diff)?;
            }
            Ok(())
        })?
    }
