...
```
```txt
Usage: op-reth proofs init [OPTIONS] --storage-path <PROOFS_HISTORY_STORAGE_PATH>
...
Static Files:
      ...
      --storage-path <PROOFS_HISTORY_STORAGE_PATH>
          The path to the storage DB for proofs history.

          This should match the path used when starting the node with `--storage-path`.
